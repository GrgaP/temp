...
void LCD_init()
{
  ...
  /*Task for initiate LCD refreshing*/
  osThreadDef(LCD_REFRESH, LCDrefresh_Task, osPriorityLow, 0, 128);
  osThreadCreate(osThread(LCD_REFRESH), NULL);
  
  /*Task for initiate frame buffer writing process from user app.*/
  osThreadDef(DSM_TASK, Disp_SM_Task, osPriorityLow+1, 0, 512);
  osThreadCreate(osThread(DSM_TASK), NULL);
}
...
{
  xSemaphoreGive(LCD_Upd_SemaphoreId);
  //HAL_LTDC_ProgramLineEvent(&hltdc, 0);//initiate event at passing of screen edge
}

/**
  * Task for switching of frame buffer. It launch of interrupt for line position event.
  */
void LCDrefresh_Task(void const * argument)
{
  ...
  for(;;){    
    if(xSemaphoreTake(LCD_Upd_SemaphoreId, portMAX_DELAY) == pdTRUE)//Waiting of signal off user app.
        HAL_LTDC_ProgramLineEvent(&hltdc, 0);//initiate event at passing of screen edge  
...
/**
  * This task begin new framebuffer editing after complete previous buffer switch procces.
  * Task must running minimum 2x more often than LCDrefresh_Task
  */
void Disp_SM_Task(void const * argument)
{
  xSemaphoreGive(LayerCH_SemaphoreId);//first launch signal
  
  for(;;){
    if(xSemaphoreTake(LayerCH_SemaphoreId, portMAX_DELAY) == pdTRUE)//Waiting signal off LCD driver
        Display_Automat();//Call user application GUI processing
...
void HAL_LTDC_LineEvenCallback(LTDC_HandleTypeDef *hltdc)
{
  ...
  xSemaphoreGiveFromISR(LayerCH_SemaphoreId,&xTaskWoken);//send signal to user app for begin writing a new frame buffer
